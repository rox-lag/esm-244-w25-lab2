...
The adjusted R^2 tells us that about 87% of the variance in data is explained by the model
...
AIC(mdl1,mdl2,mdl3)
```
We want to choose the model with the lowest AIC - it is better. 
...
select how much training data we want to use (10-fold)
...
AIC(training_lm1, training_lm2, training_lm3)
```
...
for( i in 1:folds){
  ...
  # Run for one model
  kfold_train_lm1<-lm(f1,kfold_train_df)
  
  #Get the predictions from the model
  kfold_predict_test<-kfold_test_df |> 
  mutate(model1 = predict(kfold_train_lm1,kfold_test_df))
...
```{r}
training_final_mdl2<-lm(f2,data=penguins_clean)
summary(training_final_mdl2)
```
